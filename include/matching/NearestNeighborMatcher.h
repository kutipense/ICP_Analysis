...
class NearestNeighborMatcher : public Matcher<T> {
 public:
  ...
  NearestNeighborMatcher(typename T::Ptr source_data_ptr, typename T::Ptr target_data_ptr, int number_of_checks=16, float max_distance=1, int tree_size=1) : 
  ...
  MatchList::Ptr match() override {
    ...
    float* m_flatPoints = new float[source_data_->vertices.size() * 3];
    for (size_t pointIndex = 0; pointIndex < source_data_->vertices.size(); pointIndex++) {
      for (size_t dim = 0; dim < 3; dim++) {
        m_flatPoints[pointIndex * 3 + dim] = source_data_->vertices[pointIndex][dim];
      }
    }


    flann::Matrix<float> dataset(m_flatPoints, source_data_->vertices.size(), 3);
    ...
    float* queryPoints = new float[target_data_->vertices.size() * 3];
    for (size_t pointIndex = 0; pointIndex < target_data_->vertices.size(); pointIndex++) {
      for (size_t dim = 0; dim < 3; dim++) {
        queryPoints[pointIndex * 3 + dim] = target_data_->vertices[pointIndex][dim];
      }
    }

    flann::Matrix<float> query(queryPoints, target_data_->vertices.size(), 3);
    ...
    // Filter the matches.
    const unsigned nMatches = target_data_->vertices.size();
